...
# Simple Collision Avoidance Environment - Continuous Action Space
# 2021/03/25
...
class ColAvoidEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # # LiDAR PARAMETERS
        # self.max_detection = 8                  # detection range (m)
        ...
        # self.observation_space = gym.spaces.Box( # preprocessed lidar info
        #     np.concatenate((0.0, -np.pi, -10*np.ones(2))),
        #     np.concatenate((20.0, np.pi,  10*np.ones(2))),
        #     dtype=np.float32)
    ...
    def step(self, action):
        ...
        # # observation
        # self.observation = self.observe()
        self.observation = np.array([random.randrange(-100, 100) / 10.0, 
                                     random.randrange(-100, 100) / 10.0, 
                                     random.randrange(-100, 100) / 10.0, 
                                     random.randrange(-100, 100) / 10.0]).astype(np.float32)
        
        # # reward
        # self.reward()
        self.reward = self.time_step
        self.time_step += 1

        # # done
        # self.done = bool(self.check_collision() or self.check_escape())
...
check_env(env, warn=True) # must be used before being wrapped
